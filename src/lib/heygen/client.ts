// Minimal HeyGen V2 client. We only call the few endpoints we need to
// pre-render Aria's "Meet your coach" intro video — generate, poll for
// completion, list avatars. Lives server-side only (uses HEYGEN_API_KEY).
...
type GenerateInput = {
  ...
  voiceId: string;
  text: string;
  /** 16:9 portrait of Aria. 720p keeps file size sane (~3-5 MB / minute). */
  width?: number;
  height?: number;
    ...
    video_inputs: [
      {
        ...
        background: {
          type: "color",
          value: input.backgroundHex ?? "#f5efe6",
        },
      },
    ],
    dimension: {
