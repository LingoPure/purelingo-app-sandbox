...
| ISS-049 | 2026-09-20 | Daniel Maneveld — docx §03, Review Section 08 screenshot | Org setup throws raw `organisations_slug_key` duplicate-key Postgres error when slug "prelabz" already exists; random name succeeds | Resolved (partial) | `runSelfSetup()` retries the insert with a random slug suffix on `23505` (up to 5 attempts), returns a friendly message on the non-recoverable path — the raw-error crash is fixed. The real "join existing org" flow (name match ≠ membership) is still not built; it silently mints a second org instead. | 2026-09-21, `8218c2e` (crash only) |
| ISS-050 | 2026-09-20 | Daniel Maneveld — docx §02/§04 | Target badge shows C2.1 while skill target line + My Programme show C1 | Open | Logged. Not yet scoped — worth re-checking against Phase 1's `scoreToCefrBand()`/`computeGap()` unification before assuming still broken. | — |
